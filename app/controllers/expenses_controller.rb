class ExpensesController < ApplicationController
  ...
  def index
    if params[:date] == "cash"
      @expenses = Log.find_by(:id => params[:log_id]).expenses.order(:date_cash).select("id, log_id, description, amount, date(date_cash) as ds, category_id, deprec_months, method_id, user_id, is_outlier")
