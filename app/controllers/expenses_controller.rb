class ExpensesController < ApplicationController
  ...
  def index
    if params[:date] == "cash"
      ...
      @dates = @expenses.select("date(date_cash) as date_show, sum(amount) as date_amount").group("date(date_cash)").order("date(date_cash)")
      ...
      @dates = @expenses.select("date(date_expense) as date_show, sum(amount) as date_amount").group("date(date_expense)").order("date(date_expense)")
    end
